...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/oauth/start') {
      ...
      catch (error) { console.error('[oauth-start]', error.code || '', error.message); oauth.record(request, response, error); return redirect(response, '/login/?oauth=error'); }
    }
    if (request.method === 'GET' && url.pathname === '/auth/callback') {
      try { await oauth.callback(request, response, url); return redirect(response, '/?oauth=success'); }
      catch (error) { console.error('[oauth-callback]', error.code || '', error.message); oauth.record(request, response, error); return redirect(response, '/login/?oauth=error'); }
    }
